...
__device__ void ibm_to_restruct_2_conv(unsigned char ibm[4], unsigned char to[4])
{
    to[0] = ibm[0];
    to[1] = ibm[3];
    to[2] = ibm[1];
...
__device__ void ibm_to_restruct_3_conv(unsigned char ibm[4], unsigned char to[4])
{
    to[0] = ibm[0];
    to[1] = ibm[2];
    to[2] = ibm[3];
...
__device__ void ibm_to_restruct(float* from, float* to, char type)
{
    ...
    case D_DATA_RESTRUCT_TP1:
        ibm_to_restruct_1_conv((unsigned char*)from, (unsigned char*)to);
        break;

    case D_DATA_RESTRUCT_TP2:
        ibm_to_restruct_2_conv((unsigned char*)from, (unsigned char*)to);
        break;

    case D_DATA_RESTRUCT_TP3:
        ibm_to_restruct_3_conv((unsigned char*)from, (unsigned char*)to);
        break;
...
__global__ void restructKernel(float* data, float* newData1, float* newData2, float* newData3, long int blockFileSize)
{
    ...
    if (i < blockFileSize)
    {
        ibm_to_restruct(&data[i], &newData1[i], D_DATA_RESTRUCT_TP1);
        ibm_to_restruct(&data[i], &newData2[i], D_DATA_RESTRUCT_TP2);
        ibm_to_restruct(&data[i], &newData3[i], D_DATA_RESTRUCT_TP3);
    }
}
